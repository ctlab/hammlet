...
def bootstrap_LL(
    ...
    log_info("Senior model: {}".format(model_senior.name))
    ...
    log_info(
        "Bootstraping models {}/{} (senior/junior) {} times...".format(
            model_senior.name, model_junior.name, bootstrap_times
        )
    )
    data = []
    for _ in range(bootstrap_times):
        ...
        result_boot_senior = optimizer_boot.one(model_senior, permutation)
        result_boot_junior = optimizer_boot.one(model_junior, permutation)
        LLx = result_boot_senior.LL
        LLy = result_boot_junior.LL
        LL_diff = LLx - LLy
        data.append(
            (
    ...
    headers = ["y", "Mx", "LLx", "My", "LLy", "LLx-LLy"]
