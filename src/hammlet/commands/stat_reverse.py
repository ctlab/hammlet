...
def stat_reverse(preset, y, r, critical_pvalue, method, theta0, debug):
    ...
    _, p = get_pvalue(best_result_N4, best_result_N0, df=4)
    log_info("p-value for N4-N0: {:.3f}".format(p))
    if p >= critical_pvalue:
        log_info("p >= critical_pvalue, N0 is better than N4")
        log_success("Final result: N0, model {}".format(best_result_N0.model.name))
        return
    else:
        log_info("p < critical_pvalue, N4 is still the best choice")
    del p
    ...
    _, p = get_pvalue(best_result_N4, best_result_N1, df=3)
    log_info("p-value for N4-N1: {:.3f}".format(p))
    if p >= critical_pvalue:
        log_info("p >= critical_pvalue, N1 is better than N4")
    ...
    _, p = get_pvalue(best_result_N4, best_result_N2, df=2)
    log_info("p-value for N4-N2: {:.3f}".format(p))
    if p >= critical_pvalue:
        log_info("p >= critical_pvalue, N2 is better than N4")
